...
$('.delete-category').click(function () {
    let inputCheckbox = $('.category-checked');
    let idCategoryDelete = [];
    for (let i = 0; i < inputCheckbox.length; i++) {
        if (inputCheckbox[i].checked) {
            idCategoryDelete.push(inputCheckbox[i].value)
        }
    }
    // set csrf vao header cua request
    ...
    // goi ajax
    $.ajax({
        url: origin + '/admin/categorys/delete',
        method: 'POST',
        data: {
            'categoryId': idCategoryDelete
        },
        success: function (res) {
            if (res.status === 'success') {
                for (let j = 0; j < idCategoryDelete.length; j++) {
                    $('#category-item-' + idCategoryDelete[j]).remove();
                }
            }
        },
        error: function (error) {
